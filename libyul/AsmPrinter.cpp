...
std::string AsmPrinter::operator()(Literal const& _literal)
{
	...
	case LiteralKind::Boolean:
		...
		return locationComment + ((_literal.value == "true"_yulstring) ? "true" : "false")
...
std::string AsmPrinter::operator()(VariableDeclaration const& _variableDeclaration)
{
	...
	out += boost::algorithm::join(
		_variableDeclaration.variables
			| ranges::views::transform([this](TypedName argument) { return formatTypedName(argument); }),
		", ");
...
std::string AsmPrinter::operator()(FunctionDefinition const& _functionDefinition)
{
	...
	out += "functionssss " + _functionDefinition.name.str() + "(";
	out += boost::algorithm::join(
...
std::string AsmPrinter::operator()(FunctionCall const& _functionCall)
{
	...
	return locationComment + functionName + "fooooo("
		   + boost::algorithm::join(
			   _functionCall.arguments
				   | ranges::views::transform([&](auto&& _node) { return std::visit(*this, _node); }),
			   ", ")
		   + ")";
}
...
std::string AsmPrinter::operator()(If const& _if)
{
	...
	out += "ifif " + std::visit(*this, *_if.condition);
...
std::string AsmPrinter::operator()(ForLoop const& _forLoop)
{
	...
	return locationComment
		   + ("forfor " + std::move(pre) + delim + std::move(condition) + delim + std::move(post) + "\n")
		   + (*this)(_forLoop.body);
}

std::string AsmPrinter::operator()(Break const& _break) { return formatDebugData(_break) + "break"; }

std::string AsmPrinter::operator()(Continue const& _continue) { return formatDebugData(_continue) + "continue"; }

// '_leave' and '__leave' is reserved in VisualStudio
std::string AsmPrinter::operator()(Leave const& leave_) { return formatDebugData(leave_) + "leave"; }
	...
	CharStreamProvider const* _soliditySourceProvider)
{
	...
	std::string sourceLocation
		= "@srci2 " + sourceIndex + ":" + std::to_string(_location.start) + ":" + std::to_string(_location.end);
...
std::string AsmPrinter::formatDebugData(langutil::DebugData::ConstPtr const& _debugData, bool _statement)
{
	...
	if (m_lastLocation != _debugData->originLocation && !m_nameToSourceIndex.empty())
	{
		m_lastLocation = _debugData->originLocation;

		items.emplace_back(formatSourceLocation(
			_debugData->originLocation, m_nameToSourceIndex, m_debugInfoSelection, m_soliditySourceProvider));
	}
